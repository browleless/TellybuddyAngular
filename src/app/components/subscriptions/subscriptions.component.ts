...
export class SubscriptionsComponent implements OnInit {
    ...
    compare(a, b) {
        ...
        if (
            values.indexOf(a.subscriptionStatusEnum) <=
            values.indexOf(b.subscriptionStatusEnum)
        ) {
            return -1;
        } else {
            return 1;
    ...
    ngOnInit() {
        this.subscriptionService.retrieveAllCustomerSubscriptions().subscribe(
            (response) => {
                this.subscriptions = response.subscriptions.sort(this.compare);

                for (var i = 0; i < this.subscriptions.length; i++) {
                    var sub = this.subscriptions[i];
                    if (sub.subscriptionStartDate != null) {
                        sub.subscriptionStartDate = new Date(
    ...
    loadCharts(i) {
        ...
        this.remainingData =
            (this.dataUnits *
                this.selectedSubscriptionPlan.dataConversionRate) /
                1000 -
            this.dataUsage;
        this.remainingSms =
            this.smsUnits * this.selectedSubscriptionPlan.smsConversionRate -
            this.smsUsage;
        this.remainingTalktime =
            this.talktimeUnits *
                this.selectedSubscriptionPlan.talktimeConversionRate -
            this.talktimeUsage;
        ...
        let dataDonutChart = new Chart(this.dataCtx, {
            ...
            data: {
                datasets: [
                    {
                        data: [
                            this.remainingData > 0 ? this.dataUsage : 0,
                            this.remainingData > 0 ? this.remainingData : 0,
                            this.remainingData < 0 ? this.remainingData : 0,
                        ],
                        backgroundColor: [
                            'rgba(255, 99, 132, 1)',
        ...
        let smsDonutChart = new Chart(this.smsCtx, {
            ...
            data: {
                datasets: [
                    {
                        data: [
                            this.remainingSms > 0 ? this.smsUsage : 0,
                            this.remainingSms > 0 ? this.remainingSms : 0,
                            this.remainingSms < 0 ? this.remainingSms : 0,
                        ],
                        backgroundColor: [
                            'rgba(255, 99, 132, 1)',
        ...
        this.talkTimeCanvas = document.getElementById(
            ('talkTimeChart' + i).toString()
        );
        ...
        let talkTimeDonutChart = new Chart(this.talkTimeCtx, {
            ...
            data: {
                datasets: [
                    {
                        data: [
                            this.remainingTalktime > 0 ? this.talktimeUsage : 0,
    ...
    openGroup(i: number) {
        this.subscriptionService
            .retrieveSubscriptionById(this.subscriptions[i])
            .subscribe(
                (response) => {
                    ...
                    //   }
                    if (
                        this.selectedSubscription.subscriptionStatusEnum ==
                        'ACTIVE'
                    ) {
                        this.dataUsage = this.selectedSubscription.usageDetails[
                            this.selectedSubscription.usageDetails.length - 1
                        ].dataUsage;
                        this.smsUsage = this.selectedSubscription.usageDetails[
                            this.selectedSubscription.usageDetails.length - 1
                        ].smsUsage;
                        this.talktimeUsage = this.selectedSubscription.usageDetails[
                            this.selectedSubscription.usageDetails.length - 1
                        ].talktimeUsage;
                    }
                    if (
                        this.selectedSubscription.subscriptionStartDate != null
                    ) {
                        this.currentDate = new Date();
                        this.monthAgo = new Date(this.currentDate);
                        var date = new Date(
                            this.selectedSubscription.subscriptionStartDate
                        );
                        if (this.currentDate.getDate() < date.getDate()) {
                            this.monthAgo.setMonth(
                                this.monthAgo.getMonth() - 1
                            );
                        }
                    }
                    this.loadCharts(i);
                    ...
                    if (itemsExceeded.length != 0) {
                        let snackBarRef = this.snackBar.open(
                            "You've exceeded your monthly allowance for" +
                                itemsExceeded +
                                '. Purchase a top-up now to avoid penalty charges! ',
                            'Close',
                            {
                                duration: 10000,
    ...
    requestToTerminateSubscription(i: number): void {
        if (this.subscriptions[i].subscriptionStatusEnum != 'ACTIVE') {
            ...
            this.subscriptionService
                .terminateSubscription(this.selectedSubscription)
                .subscribe(
                    (response) => {
                        this.subscriptionService
                            .retrieveAllCustomerSubscriptions()
                            .subscribe((response) => {
                                this.subscriptions = response.subscriptions.sort(
                                    this.compare
                                );
                            });
                        console.log(
                            'Request has been put in to terminate the subscription'
                        );
                        this.subscriptions;
                        this.snackBar.open(
